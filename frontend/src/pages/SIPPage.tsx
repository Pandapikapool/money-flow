...
export default function SIPPage() {
    ...
    const handleExportCSV = () => {
        ...
        const headers =
            'Name,SIP Amount,Start Date,Total Units,Current NAV,Total Invested,Current Value,Returns %,Status,Notes\n';
        const rows = items
            .map(
                (item) =>
    ...
    const fetchRealTimeNav = async (
        ...
        try {
            ...
            const response = await fetch(
                `https://api.mfapi.in/mf/search?q=${encodeURIComponent(coreTerms)}`
            );
            ...
            for (const result of results) {
                ...
                if (
                    fundWords[0] === resultWords[0] &&
                    fundWords.length > 1 &&
                    resultWords.length > 1
                ) {
                    // Same AMC, check for Direct/Regular and Growth/IDCW match
                    const hasDirectMatch =
                        fundNameLower.includes('direct') === resultNameLower.includes('direct');
                    const hasRegularMatch =
                        fundNameLower.includes('regular') === resultNameLower.includes('regular');
                    const hasGrowthMatch =
                        fundNameLower.includes('growth') === resultNameLower.includes('growth');
                    const hasIdcwMatch =
                        fundNameLower.includes('idcw') === resultNameLower.includes('idcw');
            ...
            const navResponse = await fetch(
                `https://api.mfapi.in/mf/${bestMatch.schemeCode}/latest`
            );
    ...
    const handleRefreshAllNav = async () => {
        ...
        if (failedCount > 0) {
            alert(
                `Updated ${ongoing.length - failedCount}/${ongoing.length} funds. ${failedCount} fund(s) need manual NAV update.`
            );
        }
    };
    ...
    const ongoingItems = items
        .filter((i) => i.status === 'ongoing' || i.status === 'paused')
        .sort((a, b) => a.name.localeCompare(b.name));
    const redeemedItems = items
    ...
    const handleRedeem = async (e: React.FormEvent) => {
        ...
        try {
            await redeemSIP(redeemingItem.id, parseFloat(redeemAmount), redeemDate);
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <button
                        ...
                        style={{
                            ...
                            borderRadius: '6px',
                            cursor:
                                ongoingItems.length === 0 || refreshingNav !== null
                                    ? 'not-allowed'
                                    : 'pointer',
                            fontSize: '0.8rem',
                    ...
                    >
                        {refreshingNav !== null ? <>Updating...</> : <>&#8635; Refresh NAV</>}
                    </button>
                    {lastNavUpdate && (
                        ...
                        {ongoingItems.map((item) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {item.returns_percent >= 0 ? '+' : ''}
                                        {item.returns_percent.toFixed(1)}%
                                    </span>
                                </td>
                                <td
                                    style={{
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            ...
                                            style={{
                                                padding: '5px 8px',
                                                background: item.scheme_code
                                                    ? 'rgba(16, 185, 129, 0.1)'
                                                    : 'transparent',
                                                border: item.scheme_code
                                                    ? '1px solid rgba(16, 185, 129, 0.3)'
                                                    : '1px solid var(--border-color)',
                                                borderRadius: '4px',
                                                cursor:
                                                    refreshingNav === item.id ? 'wait' : 'pointer',
                                                fontSize: '0.7rem',
                                                color: item.scheme_code
                                                    ? 'var(--accent-success)'
                                                    : 'var(--text-primary)',
                                            ...
                                            title={
                                                item.scheme_code
                                                    ? 'Refresh NAV (linked to mfapi.in)'
                                                    : 'Refresh NAV (search by name)'
                                            }
                                        >
                            ...
                            <tr key={item.id} style={{ opacity: 0.6 }}>
                                ...
                                >
                                    ...
                                    >
                                        Redeemed{' '}
                                        {item.redeemed_date
                                            ? formatDateShort(item.redeemed_date)
                                            : '-'}
                                    </div>
                                </td>
                                <td
                ...
                {!isCreating ? (
                    ...
                    >
                        <div
                            style={{
                                display: 'grid',
                                gridTemplateColumns: '1fr 100px 120px 100px 100px 1fr auto',
                                gap: '12px',
                                alignItems: 'flex-end',
                            }}
                        >
                            <div style={{ position: 'relative' }}>
                                ...
                                >
                                    Fund Name{' '}
                                    {searchingFunds && (
                                        <span style={{ marginLeft: '8px', fontSize: '0.65rem' }}>
                                            searching...
                                        </span>
                                    )}
                                    {selectedSchemeCode && (
                                ...
                                {showSuggestions &&
                                    ...
                                    dropdownPosition && (
                                        ...
                                        >
                                            ...
                                            >
                                                Don't see your fund? Just type the name and press
                                                Add
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div>
                                ...
                                >
                                    Current NAV <span style={{ opacity: 0.6 }}>(optional)</span>
                                </label>
                                <input
                                    type="number"
                                    step="0.01"
                            ...
                            <div>
                                ...
                                >
                                    Total Units <span style={{ opacity: 0.6 }}>(optional)</span>
                                </label>
                                <input
                                    type="number"
                                    step="0.0001"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span style={{ fontWeight: '600' }}>
                                        {formatCurrency(editingItem.total_invested)}
                                    </span>
                                </div>
                                <div
                                    style={{
                                ...
                                >
                                    ...
                                    <span
                                        style={{
                                            fontWeight: '600',
                                            color:
                                                editingItem.returns_percent >= 0
                                                    ? 'var(--accent-success)'
                                                    : 'var(--accent-danger)',
                                        }}
                                    >
                                        {formatCurrency(
                                            parseFloat(editUnits) * parseFloat(editNav) ||
                                                editingItem.current_value
                                        )}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    >
                                        {editingItem.returns_percent >= 0 ? '+' : ''}
                                        {editingItem.returns_percent.toFixed(2)}%
                                    </span>
                                </div>
                            </div>
                ...
                >
                    ...
                    >
                        <h3 style={{ margin: '0 0 20px' }}>
                            Add Installment - {installmentSIP.name}
                        </h3>
                            ...
                            <div style={{ marginBottom: '16px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    ...
                                    >
                                        <input
                                            type="radio"
                                            checked={installmentType === 'sip'}
                                            onChange={() => setInstallmentType('sip')}
                                        />
                                        Regular SIP
                                    </label>
                                    <label
                                    ...
                                    >
                                        <input
                                            type="radio"
                                            checked={installmentType === 'lumpsum'}
                                            onChange={() => setInstallmentType('lumpsum')}
                                        />
                                        Lumpsum
                                    </label>
                                </div>
                        ...
                        <form onSubmit={handleRedeem}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span style={{ fontWeight: '600' }}>
                                        {formatCurrency(redeemingItem.total_invested)}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ color: 'var(--text-secondary)' }}>
                                    ...
                                    >
                                        {formatCurrency(redeemingItem.current_value)}
                                    </span>
                                </div>
                            </div>
                            <div style={{ marginBottom: '16px' }}>
