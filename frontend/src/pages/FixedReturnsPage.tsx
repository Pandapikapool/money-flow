...
export default function FixedReturnsPage() {
    ...
    const handleExportCSV = () => {
        ...
        const headers =
            'Name,Invested Amount,Interest Rate %,Start Date,Maturity Date,Expected Withdrawal,Actual Withdrawal,Status,Closed Date,Notes\n';
        const rows = items
            .map(
                (item) =>
        ...
        link.setAttribute(
            'download',
            `fixed_returns_${new Date().toISOString().split('T')[0]}.csv`
        );
    ...
    const closedItems = items
        ...
        .sort(
            (a, b) =>
                new Date(b.closed_date || b.maturity_date).getTime() -
                new Date(a.closed_date || a.maturity_date).getTime()
        );
    ...
    const handleClose = async (e: React.FormEvent) => {
        ...
        try {
            await closeFixedReturn(closingItem.id, parseFloat(closeWithdrawal), closeDate);
            setClosingItem(null);
            loadData();
        } catch (err) {
                    ...
                    <tbody>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <input
                                            ...
                                            style={{ width: '130px', marginBottom: '4px' }}
                                        />
                                        <div
                                            style={{
                                                fontSize: '0.7rem',
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {isMaturingSoon(item.maturity_date) && (
                                                ...
                                                >
                                                    {getDaysUntilMaturity(item.maturity_date) <= 0
                                                        ? 'MATURED'
                                                        : `${getDaysUntilMaturity(item.maturity_date)}d`}
                                                </span>
                                            )}
                                        </div>
                                    </td>
                                    ...
                                    <td
                                        style={{
                                            padding: '16px',
                                            borderBottom: '1px solid var(--border-color)',
                                            color: isMaturingSoon(item.maturity_date)
                                                ? 'rgb(180, 120, 20)'
                                                : 'inherit',
                                            fontWeight: isMaturingSoon(item.maturity_date)
                                                ? '600'
                                                : 'normal',
                                        }}
                                    >
                        ...
                        {closedItems.map((item) =>
                            editingClosedId === item.id ? (
                                <tr
                                    key={item.id}
                                    style={{ background: 'rgba(99, 102, 241, 0.05)', opacity: 0.8 }}
                                >
                                    <td
                                        style={{
                                            padding: '12px 16px',
                                    ...
                                    >
                                        <input
                                            type="number"
                                            value={editClosedWithdrawal}
                                            onChange={(e) =>
                                                setEditClosedWithdrawal(e.target.value)
                                            }
                                            style={{ width: '100px', textAlign: 'right' }}
                                        />
                                    </td>
                                    ...
                                    >
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            ...
                                            <button
                                                onClick={() => setEditingClosedId(null)}
                                                style={{
                                                    padding: '6px 12px',
                                                    background: 'transparent',
                                                    border: '1px solid var(--border-color)',
                ...
                {!isCreating ? (
                    ...
                    >
                        <div
                            style={{
                                display: 'grid',
                                gridTemplateColumns: '1fr 120px 80px 130px 130px 1fr auto',
                                gap: '12px',
                                alignItems: 'flex-end',
                            }}
                        >
