...
export default function RecurringDepositsPage() {
    ...
    const handleExportCSV = () => {
        ...
        const headers =
            'Name,Installment,Frequency,Rate %,Start Date,Paid/Total,Remaining,Invested,Maturity,Status,Notes\n';
        const rows = items
            .map(
                (item) =>
        ...
        link.setAttribute(
            'download',
            `recurring_deposits_${new Date().toISOString().split('T')[0]}.csv`
        );
    ...
    const ongoingItems = items
        .filter((i) => i.status === 'ongoing' || i.status === 'completed')
        .sort((a, b) => {
    ...
    const handleClose = async (e: React.FormEvent) => {
        ...
        try {
            await closeRecurringDeposit(closingItem.id, parseFloat(closeAmount), closeDate);
            setClosingItem(null);
            loadData();
        } catch (err) {
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {item.custom_frequency_days
                                                ? ` (${item.custom_frequency_days} days)`
                                                : ''}
                        ...
                        {closedItems.map((item) => (
                            <tr key={item.id} style={{ opacity: 0.6 }}>
                                ...
                                >
                                    ...
                                    >
                                        Closed{' '}
                                        {item.closed_date ? formatDate(item.closed_date) : '-'}
                                    </div>
                                </td>
                                <td
                                    style={{
                ...
                {!isCreating ? (
                    ...
                    >
                        <div
                            style={{
                                display: 'grid',
                                gridTemplateColumns: '1fr 100px 120px 80px 120px 80px 1fr auto',
                                gap: '12px',
                                alignItems: 'flex-end',
                            }}
                        >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setNewFrequency(
                                            e.target.value as 'monthly' | 'yearly' | 'custom'
                                        )
                                    }
                                    style={{ width: '100%' }}
                                >
                        ...
                        {newFrequency === 'custom' && (
                            <div style={{ marginTop: '12px' }}>
                                ...
                                <input
                                    ...
                                    placeholder="e.g., 15 for bi-weekly"
                                    style={{ width: '150px' }}
                                />
                            </div>
                        )}
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span style={{ fontWeight: '600' }}>
                                        {formatCurrency(closingItem.total_invested)}
                                    </span>
                                </div>
                                <div
                                    style={{
                                ...
                                >
                                    ...
                                    >
                                        {formatCurrency(closingItem.maturity_value)}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <span style={{ fontWeight: '600' }}>
                                        {closingItem.installments_paid} /{' '}
                                        {closingItem.total_installments}
                                    </span>
                                </div>
                            </div>
