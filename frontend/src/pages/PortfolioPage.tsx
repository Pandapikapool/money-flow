...
export default function PortfolioPage() {
    ...
    const slices: Slice[] = [
        {
            name: 'Fixed Returns',
            invested: fixed?.total_invested ?? 0,
            current: fixed?.total_expected ?? 0,
            path: '/investments/fixed',
            color: COLORS[0],
        },
        {
            name: 'SIP / MF',
            invested: sip?.total_invested ?? 0,
            current: sip?.current_value ?? 0,
            path: '/investments/sip',
            color: COLORS[1],
        },
        {
            name: 'Rec. Deposits',
            invested: rd?.total_invested ?? 0,
            current: rd?.total_maturity ?? 0,
            path: '/investments/rd',
            color: COLORS[2],
        },
        {
            name: 'Indian Stocks',
            invested: indianStocks?.total_invested ?? 0,
            current: indianStocks?.current_value ?? 0,
            path: '/investments/stocks/indian',
            color: COLORS[3],
        },
        {
            name: 'US Stocks',
            invested: usStocks?.total_invested ?? 0,
            current: usStocks?.current_value ?? 0,
        ...
        {
            name: 'Crypto',
            invested: crypto?.total_invested ?? 0,
            current: crypto?.current_value ?? 0,
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
            ...
            >
                ...
                <div
                    ...
                    style={{ padding: '22px', borderLeft: '4px solid var(--accent-primary)' }}
                >
                    <div
                        style={{
                            fontSize: '0.75rem',
                    ...
                    <div style={{ fontSize: '1.8rem', fontWeight: '800' }}>
                        {formatCurrency(totalCurrentINR)}
                    </div>
                    <div
                        style={{
                            fontSize: '0.85rem',
                    ...
                    <div
                        style={{
                            ...
                            fontSize: '0.95rem',
                            fontWeight: '700',
                            color: gainINR >= 0 ? 'var(--accent-success)' : 'var(--accent-danger)',
                        }}
                    >
                        {gainINR >= 0 ? '+' : ''}
                        {formatCurrency(gainINR)} ({gainPctINR >= 0 ? '+' : ''}
                        {gainPctINR.toFixed(1)}%)
                    </div>
                </div>
                ...
                {totalInvestedUSD > 0 && (
                    <div
                        className="glass-panel"
                        style={{ padding: '22px', borderLeft: '4px solid #60a5fa' }}
                    >
                        <div
                            style={{
                                fontSize: '0.75rem',
                        ...
                        <div
                            style={{
                                ...
                                fontWeight: '700',
                                color:
                                    gainUSD >= 0 ? 'var(--accent-success)' : 'var(--accent-danger)',
                            }}
                        >
                            {gainUSD >= 0 ? '+' : ''}
                            {fmtUSD(gainUSD)} ({gainPctUSD >= 0 ? '+' : ''}
                            {gainPctUSD.toFixed(1)}%)
                        </div>
                    </div>
                )}
                ...
                <div
                    className="glass-panel"
                    style={{ padding: '22px', borderLeft: '4px solid var(--accent-success)' }}
                >
                    <div
                        style={{
                            fontSize: '0.75rem',
                    ...
                    >
                        {[
                            ...
                            sip?.ongoing_count,
                            rd?.ongoing_count,
                            indianStocks?.holding_count,
                            usStocks?.holding_count,
                            crypto?.holding_count,
                        ]
                            .filter(Boolean)
                            .join(' + ')}{' '}
