...
const getLifeXpActionLabel = (action: ActivityLogEntry['action']): string => {
    switch (action) {
        ...
        case 'bucket_achieved':
            return 'Bucket Achieved';
        case 'bucket_reactivated':
            return 'Bucket Reactivated';
        case 'contribution_added':
            return 'Contribution Added';
        case 'contribution_marked_done':
            return 'Contribution Marked Done';
...
const exportLifeXpLogAsCSV = (log: ActivityLogEntry[]) => {
    ...
    link.setAttribute(
        'download',
        `lifexp_activity_log_${new Date().toISOString().split('T')[0]}.csv`
    );
...
export default function LifeXpPage() {
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (selectedBucket.target_amount !== updated.target_amount)
                changes.push(`Target: ${formatCurrency(updated.target_amount)}`);
            if (selectedBucket.is_repetitive !== updated.is_repetitive)
                changes.push(`Repetitive: ${updated.is_repetitive}`);
            if (selectedBucket.contribution_frequency !== updated.contribution_frequency)
                changes.push(
                    `Frequency: ${getFrequencyLabel(updated.contribution_frequency, updated.custom_frequency_days)}`
                );
            addToLifeXpLog({
                date: new Date().toISOString().split('T')[0],
    ...
    const actionNeededBuckets = activeBuckets.filter(
        (b) => b.is_repetitive && (isContributionDue(b) || isContributionOverdue(b))
    );
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
                <button
                    ...
                    style={{
                        padding: '8px 16px',
                        background:
                            viewTab === 'active' ? 'var(--accent-primary)' : 'var(--bg-panel)',
                ...
                <button
                    ...
                    style={{
                        padding: '8px 16px',
                        background:
                            viewTab === 'achieved' ? 'var(--accent-success)' : 'var(--bg-panel)',
                ...
                {displayBuckets.map((bucket) => {
                    const progress =
                        bucket.target_amount > 0
                            ? (bucket.saved_amount / bucket.target_amount) * 100
                            : 0;
                            ...
                            <div style={{ marginBottom: '12px' }}>
                                ...
                                >
                                    <span style={{ color: 'var(--text-secondary)' }}>
                                        {formatCurrency(bucket.saved_amount)} /{' '}
                                        ...
                                        >
                                            ...
                                            {newFrequency === 'custom' && (
                                                <input
                                                    ...
                                                    value={newCustomDays}
                                                    onChange={(e) =>
                                                        setNewCustomDays(e.target.value)
                                                    }
                                                    placeholder="Days"
                                                    min="1"
                                                />
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                ...
                                <tbody>
                                    {activityLog.map((entry) => {
                                        const actionColor =
                                            entry.action === 'contribution_added' ||
                                            entry.action === 'contribution_marked_done'
                                                ? 'var(--accent-success)'
                                                : entry.action === 'bucket_created'
                                                  ? 'var(--accent-primary)'
                                                  : entry.action === 'bucket_updated'
                                                    ? 'var(--accent-warning)'
                                                    : entry.action === 'bucket_deleted' ||
                                                        entry.action === 'history_deleted'
                                                      ? 'var(--accent-danger)'
                                                      : entry.action === 'bucket_achieved'
                                                        ? 'var(--accent-success)'
                                                        : 'var(--text-secondary)';
            ...
            {selectedBucket && (
                ...
                >
                    ...
                    >
                            ...
                            >
                                <div
                                    style={{
                                        ...
                                        background:
                                            selectedBucket.saved_amount >=
                                            selectedBucket.target_amount
                                                ? 'var(--accent-success)'
                                                : 'var(--accent-primary)',
                                        borderRadius: '6px',
                                        transition: 'width 0.3s',
                        ...
                        {/* History */}
                        <h3 style={{ fontSize: '1rem', margin: '0 0 16px 0' }}>
